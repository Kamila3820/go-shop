...
func (f *userReq) Customer() (IInsertUser, error) {
	...
	defer cancel() // return คืนโดยที่จะไม่มีตัวแปรค้างอยู่ข้างใน
	...
	if err := f.db.QueryRowContext(
		...
		f.req.Password,
		f.req.Username,
		// force ให้ role_id เป็น 1 เลย
	).Scan(&f.id); err != nil { // .Scan() เพราะมี RETURNING ใน query
		switch err.Error() {
...
func (f *userReq) Result() (*users.UserPassport, error) {
	...
	if err := json.Unmarshal(data, &user); err != nil { //แปลง byteเป็น json structของuser

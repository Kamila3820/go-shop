...
func (b *updateProductBuilder) updateTitleQuery() {
	if b.req.Title != "" {
		b.values = append(b.values, b.req.Title) //append เข้าไป valueมีlenเป็น1
		b.lastStackIndex = len(b.values)

		b.queryFields = append(b.queryFields, fmt.Sprintf(`
		"title" = $%d`, b.lastStackIndex)) //$%d จะเท่ากับ $1
	}
}
...
func (b *updateProductBuilder) updateCategory() error {
	...
	if _, err := b.tx.ExecContext(
		...
		b.tx.Rollback()
		return fmt.Errorf("update product_category failed: %v", err)
	}
	...
	if len(images) > 0 {
		...
		for _, img := range images {
			deleteFileReq = append(deleteFileReq, &files.DeleteFileReq{
				Destination: fmt.Sprintf("image/product/%s", img.FileName),
			})
		}
